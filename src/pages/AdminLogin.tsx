...
export function AdminLogin() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Supabase auth irá aqui nas próximas etapas.
    ...
    <div className="bg-surface font-body text-on-surface min-h-screen flex flex-col selection:bg-primary-fixed selection:text-on-primary-fixed">
      ...
      <header className="fixed top-0 w-full z-50 bg-slate-50/70 dark:bg-slate-900/70 backdrop-blur-xl border-b border-slate-200/10 dark:border-slate-800/10 shadow-sm dark:shadow-none">
        <div className="flex justify-between items-center px-6 py-4 w-full max-w-7xl mx-auto">
          <div className="text-lg font-bold tracking-tighter text-slate-900 dark:text-slate-100">André Ric - Admin Area</div>
      ...
      <main className="flex-grow flex flex-col items-center justify-center px-6 relative overflow-hidden pt-20">
        ...
        <div className="w-full max-w-md glass-card rounded-lg p-10 shadow-[0_20px_40px_rgba(15,23,42,0.04)] border border-white/20 bg-white/70 backdrop-blur-xl">
          <div className="text-center mb-10">
            <h1 className="font-headline font-bold text-2xl tracking-tighter text-on-surface mb-2">André Ric - Admin Area</h1>
            <p className="text-on-surface-variant text-sm font-medium tracking-tight">Access the Precision Dashboard</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button type="submit" className="w-full bg-gradient-to-br from-primary to-primary-container text-on-primary font-semibold py-4 rounded-full shadow-lg shadow-primary/10 hover:-translate-y-0.5 hover:shadow-xl active:scale-95 transition-all duration-200">
              Sign In
            </button>
          </form>
        ...
        <div className="mt-10 flex items-center gap-2 group cursor-pointer" onClick={() => navigate('/')}>
          ...
          <span className="text-sm font-medium text-primary hover:text-on-primary-fixed-variant transition-colors">Return to Portfolio</span>
        </div>
      </main>
      ...
      <footer className="w-full py-8 mt-auto bg-transparent">
        <div className="flex flex-col md:flex-row justify-between items-center px-12 w-full">
          <div className="font-['Inter'] text-[11px] uppercase tracking-[0.05em] font-medium text-slate-400 dark:text-slate-500">
              © 2026 André Ric. Built for Precision.
          </div>
          <div className="flex gap-8 mt-4 md:mt-0">
            <a href="#" className="font-['Inter'] text-[11px] uppercase tracking-[0.05em] font-medium text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-slate-100 transition-all">Privacy Policy</a>
            <button onClick={() => navigate('/')} className="font-['Inter'] text-[11px] uppercase tracking-[0.05em] font-medium text-blue-600 dark:text-blue-400 underline decoration-2 underline-offset-4 hover:text-slate-900 dark:hover:text-slate-100 transition-all">Return to Portfolio</button>
          </div>
        </div>
      </footer>
    </div>
